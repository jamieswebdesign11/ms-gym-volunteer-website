...
class ECWD {
    ...
    private function __construct() {
	 ...
	 add_action('init', array($this, 'ecwd_check_theme'), 1);
	 add_filter('body_class', array($this, 'theme_body_class'));

	 add_action('wp_enqueue_scripts', array($this, 'enqueue_scripts'), 5);
	 add_action('wp_enqueue_scripts', array($this, 'enqueue_styles'));
	 add_action('ecwd_reset_template', array($this, 'reset_template'));
    ...
    // sync styles between free and premium versions, since 5.1.14
    public function ecwd_check_theme(){
        $ecwd_old_version = get_option("ecwd_version");
        $ecwd_new_version = "5.1.14"; /// keep hardcoded

        $ecwd_version_compare = 1;
        if($ecwd_old_version !== false){
          $ecwd_version_compare  = version_compare($ecwd_new_version, $ecwd_old_version);
        }
        if($ecwd_version_compare > 0 ){
          $ecwd_grey_theme = get_page_by_title("Grey", 'OBJECT', "ecwd_theme");
          if(is_null($ecwd_grey_theme )){
            return;
          }
          $ecwd_theme_meta = json_decode(get_post_meta($ecwd_grey_theme->ID, "ecwd_theme_params", true), true);
          $ecwd_theme_meta["ecwd_cal_header_border_color"]="#E5E5E5";
          update_post_meta($ecwd_grey_theme->ID, "ecwd_theme_params", json_encode($ecwd_theme_meta));
        }
    }
    public function show_related_events($events, $upcoming_events = false) {
        ...
        if(!empty($events)) {
          ...
          $events_count = intval(get_option('posts_per_page'));
          if($events_count === 0){
            $events_count = 10;
          }
          $events = array_slice($events,0, $events_count);
        }
    ...
    public static function includes() {
      ...
      include_once('includes/ecwd-google-events-class.php');
      include_once('includes/ecwd-notices-class.php');
      include_once('includes/ecwd-facebook-events-class.php');
      include_once('ics-parser/class.iCalReader.php');
      include_once('includes/ecwd-ical-events-class.php');
    ...
    public function enqueue_scripts() {
        ...
        if (is_object($post) && $load_map === true && !empty($gmap_key)) {
            if (isset($post->post_type) && ( $post->post_type == 'ecwd_event' || $post->post_type == 'ecwd_venue' || strpos($post->post_content, 'ecwd id') !== false || strpos($post->post_content, 'EcwdViews') !== false )) {
        ...
        if(!empty($gmap_key) && $load_map === true) {
            wp_enqueue_script($this->prefix . '-gmap-public', plugins_url('js/gmap/gmap3.js', __FILE__), array('jquery'), $scripts_key);
        }
        wp_enqueue_script($this->prefix . '-popup', plugins_url('js/ecwd_popup.js', __FILE__), array('jquery'), $scripts_key);
        wp_enqueue_script($this->prefix . '-public', plugins_url('js/scripts.js', __FILE__), array('jquery', 'jquery-ui-draggable', 'masonry', $this->prefix . '-popup'), $scripts_key);
        $gmap_redirect = ((isset($ecwd_options['gmap_marker_click']) && $ecwd_options['gmap_marker_click'] == '1'));
    ...
    public function reset_template($type) {
        $html = file_get_contents(ECWD_DIR . "/email-templates/" . $type . ".html");
        if (defined('ECWD_SUBSCRIBE_URL'))
            $html = str_replace('{ECWD_SUBSCRIBE_URL}', ECWD_SUBSCRIBE_URL, $html);
        if (defined('ECWD_RSVP_URL'))
            $html = str_replace('{ECWD_RSVP_URL}', ECWD_RSVP_URL, $html);
        if (defined('ECWD_TICKET_URL'))
            $html = str_replace('{ECWD_TICKET_URL}', ECWD_TICKET_URL, $html);
        update_option($type, $html);
    }
    ...
    public static function get_time_zone( $all_day = false ) {
	  ...
	  $show_timezone = ($ecwd_options["show_time_zone"] == 1) ? true : false;
  ...
  public static function add_private_posts(&$posts, $args){
