...
(function ($) {
  ...
  function show_fields(el) {
    ...
    if (el.attr('id') != 'ecwd_event_repeat_dont_repeat_radio') {
      ...
      $(".ecwd_exception_date").show();
    }
    else {
      $("#ecwd_repeat_until").hide();
      $(".ecwd_exception_date").hide();
    }
  ...
  $(document).on('click', '.ecwd-events .ecwd-calendar-event-delete', function () {
    if (confirm('Sure?')) {
      ...
      $.post(ecwd_admin_params.ajaxurl, {
        ...
        nonce : ecwdServerVars.ajaxnonce,
        ...
        if (res.status == 'ok') {
          ...
          $(element).find('input').addClass('ecwd-calendar-excluded-event-id');
        }
      });
    }
  });
  ...
  jQuery('.ecwd_show_more_updates').click(function () {
    if ($('.ecwd_more_updates').is(':visible') == false) {
      $(this).text('Show less');
    } else {
      $(this).text('More updates');
    }
    $('.ecwd_more_updates').slideToggle();
    return false;
  });


  $('.ecwd_mail_popup_container').ecwd_popup({
    button: $('#mail_template').find('#preview_mail'),
    title: $('#mail_template').find('.ecwd_post_title').find('h2').text(),
    fillhtml: $("#mail_template").find('.wp-editor-area'),
    container_class: 'ecwd_prview_mail',
    get_ajax_data: function (el) {
      if ($("[name='ecwd_action']").val() == '' || $('#ecwd_events_list').length == 0) {
        return false;
      }
      var data = {
        'action': $("[name='ecwd_action']").val(),
        'nonce' : ecwdServerVars.ajaxnonce,
        'template': $("[name='ecwd_template']").val(),
        'event_id': $('#ecwd_events_list').val()
      }
      return data;
    }
  });
/*  $('.ecwd_add_event_to_calendar').ecwd_popup({
  ...
  jQuery('.ecwd_event_repeat_event_radio').each(function () {
    if (jQuery(this).is(':checked')){
      var checked = null;
      if(jQuery(this).attr('id') == 'ecwd_repeat_event_monthly'){
        jQuery('.ecwd_repeat_advanced #ecwd_monthly input[name="ecwd_event_repeat_month_on_days"]').each(function () {
          if (jQuery(this).is(':checked')) {
            checked = this;
          }
        });
      }else if(jQuery(this).attr('value') == 'yearly'){
        jQuery('.ecwd_repeat_advanced #ecwd_yearly input[name="ecwd_event_repeat_year_on_days"]').each(function () {
          if (jQuery(this).is(':checked')) {
            checked = this;
          }
        });
  ...
  // jQuery('.ecwd_repeat_advanced  input[type="radio"]').each(function () {
  //   if (jQuery(this).is(':checked') && !jQuery(this).closest('.ecwd_event_repeat_event_div').parent().hasClass('ecwd_hidden')) {
  //     jQuery(this).trigger('click');
  //   }
  // });

    //do not allow removing of the last theme
    if (jQuery('body').hasClass('post-type-ecwd_theme')) {

        var trash_links = jQuery('#the-list .status-publish .submitdelete');
        if (trash_links.length === 1) {
            trash_links.on('click', function (e) {
                e.preventDefault();
                alert('You cannot delete the only theme');
                return false;
            })
        }
    ...
    $('.ecwd_exception_input').datetimepicker({
        scrollMonth: false,
        scrollInput: false,
        timepicker:false,
    ...
    jQuery("#ecwd_add_edate").click(function (e) {
        e.preventDefault();
        jQuery(".ecwd_exception_date .ecwd_exception_inputs").append("<div class='ecwd_input_block'><input class='ecwd_exception_input' type='text' name='ecwd_exception_date[]' autocomplete='off'> <span class='delete_exception_input dashicons dashicons-no'></span></div>")
    ...
    jQuery('body').on('click', '.delete_exception_input', function (){
        jQuery(this).closest(".ecwd_input_block").remove();
    });
...
function initialize() {
  ...
  if (ecwd_admin_params.gmap_type == 'SATELLITE') {
  ...
  radio_open : function(element){
	...
	for (i = 0; i < element.show.length; i++) {
	  for (j = 0; j < element.show[i].val.length; j++) {
		jQuery('[id$="['+element.show[i].val[j] + ']"]').closest("tr").hide();
	  }
	}
	...
	for (i = 0; i < element.show.length; i++) {
	  if(element.show[i].key == sel){
		for (j = 0; j < element.show[i].val.length; j++) {
		  jQuery('[id$="['+element.show[i].val[j] + ']"]').closest("tr").show();
		}
	  }
	}
	...
	for (i = 0; i < element.hide.length; i++) {
	  for (j = 0; j < element.hide[i].val.length; j++) {
		jQuery('[id$="['+element.hide[i].val[j] + ']"]').closest("tr").hide();
	  }
	}
  }
};
