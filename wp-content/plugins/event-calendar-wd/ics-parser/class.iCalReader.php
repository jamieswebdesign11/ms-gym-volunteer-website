...
class ICalReader {
    ...
    public function __construct($url) {
        ...
        try {
            ...
            $response = wp_remote_retrieve_body($raw_data);

            if ($response) {
                $tmpfname = tempnam(get_temp_dir(), "ecwd");
                $handle = fopen($tmpfname, "w");

                fwrite($handle, $response);
                fclose($handle);
                $lines = file($tmpfname, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                if (stristr($lines[0], 'BEGIN:VCALENDAR') === false) {
					add_settings_error('ecwd_ical',  esc_attr( 'read_ical' ), __( 'Calendar data has wrong format', 'event-calendar-wd' ));
                    return false;
                } else {
                ...
                if(is_wp_error($raw_data)){
                    add_settings_error('ecwd_ical',  $raw_data->get_error_code(), __("Cannot find any calendar with this URL","event-calendar-wd"));
                }
                return;
            }
        } catch (Exception $e) {
        ...
        switch ($component) {
            ...
            case "VEVENT":
				if($keyword === "DESCRIPTION"){
					$this->cal[$component][$this->event_count - 1][$keyword] = str_replace("\\n", "<br />", $value);	
				} else {
