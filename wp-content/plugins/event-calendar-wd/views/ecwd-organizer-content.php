...
$events = $d->get_event_days($events, 1, ECWD::ecwd_date('Y-m-d', time()), $max_date);
...
<div class="ecwd-organizer">
    ...
    if(!empty($organizer_phone_html)){
        echo sprintf($organizer_phone_html, "ecwd_organizer_phone" , esc_html($organizer_phone));
    }

    if(!empty($organizer_website_html)){
        echo sprintf($organizer_website_html, "ecwd_organizer_website", esc_url($organizer_website), esc_html($organizer_website));
    }
    
    echo '<div class="ecwd_organizer_description">' . wpautop($post->post_content) . '</div>';
    ?>

    <div class="ecwd-organizer-ext"><?php do_action('ecwd_view_ext') ?></div>
