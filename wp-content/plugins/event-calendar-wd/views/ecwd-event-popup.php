...
$category_and_tags = ($ecwd_options['category_and_tags'] != '') ? $ecwd_options['category_and_tags'] : false;
...
<div id="ecwd-events-content" class="ecwd-events-single">
  <div id="post-<?php echo esc_attr($post_id); ?>">
    ...
    <div class="ecwd-event" itemscope itemtype="http://schema.org/Event">
      ...
      <div class="event-detalis">
        ...
        <div class="ecwd-event-details">
          ...
          <?php if ($ecwd_options['show_repeat_rate'] && !$is_default_dates) { ?>
            <div class="ecwd_repeat_rate_text">
              <span><?php echo esc_html($d->get_repeat_rate($post_id, '', $date_format)); ?></span>
            </div>
          <?php }
          ...
          if ($ecwd_event_url) {
            ...
            ?>
            <div class="ecwd-url">
              <a href="<?php echo esc_url($ecwd_event_url); ?>" target="_blank">
                <label class="ecwd-event-url-info" title="<?php _e('Url', 'event-calendar-wd'); ?>"></label>
                <?php echo esc_html($ecwd_event_url); ?>
              </a>
            </div>
          <?php }
              ...
              <?php if (count($organizers) > 1) { ?>
                ...
                if (!empty($organizer_phone)) { ?>
                  <div class="ecwd_organizer_phone">
                    ...
                    <span><?php echo esc_html($organizer_phone); ?></span>
                  </div>
                <?php }
          ...
          <div class="event-venue" itemprop="location" itemscope itemtype="http://schema.org/Place">
            <?php if ($venue_post_id) { ?>
              ...
              <span itemprop="name">
                <a href="<?php echo esc_url($venue_permalink) ?>"><?php echo esc_html($venue); ?></a>
              </span>
              <?php
              ...
              if (!empty($ecwd_event_location)) {
                ...
                <div class="address" itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
                  ...
                  <span><?php echo esc_html($ecwd_event_location); ?></span>
                </div>
                <?php
              }
              ?>
            <?php } elseif ($ecwd_event_location) { ?>
              <span class="ecwd_hidden" itemprop="name"><?php echo esc_html($ecwd_event_location); ?></span>
              <label class="ecwd-venue-info" title="<?php _e('Location', 'event-calendar-wd'); ?>"></label>
              <span class="address" itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
                <?php echo esc_html($ecwd_event_location); ?>
              </span>
            <?php } ?>
          </div>
      ...
      if ($ecwd_event_show_map == 1 && $ecwd_event_latlong) {
        $map_events = array();
        $map_events[0]['latlong'] = explode(',', esc_html($ecwd_event_latlong));
        $url_for_google_map = '';
        if ($ecwd_event_location != '') {
          $map_events[0]['location'] = esc_html($ecwd_event_location);
        ...
        $map_events[0]['infow'] .= '<span class="location">' . esc_html($ecwd_event_location) . '</span>';
        ...
        <div class="ecwd-show-map">
          ...
          <textarea class="hidden ecwd_markers" style="display: none;"><?php echo esc_html($markers); ?></textarea>
        </div>
      <?php } ?>
        ...
        <div class="ecwd-event-video">
          ...
          if (strpos($ecwd_event_video, 'youtube') > 0) {
            ...
            if (isset($video_array_of_vars['v']) && $video_array_of_vars['v']) {
              ?>
              <object data="//www.youtube.com/v/<?php echo esc_html($video_array_of_vars['v']); ?>"
                      type="application/x-shockwave-flash" width="400" height="300">
                <param name="src"
                       value="https://www.youtube.com/v/<?php echo esc_html($video_array_of_vars['v']); ?>"/>
              </object>
              <?php
            }
            ...
            if ($videoID) {
              ?>
              <iframe
                src="https://player.vimeo.com/video/<?php echo esc_html($videoID); ?>?title=0&amp;byline=0&amp;portrait=0&amp;badge=0&amp;color=ffffff"
            ...
            <ul class="event_categories">
              <?php foreach ($event_categories as $category) {
                ...
                <li class="event_category event-details-title">
                  <?php if ($metas['color']) { ?>
                    <span class="event-metalabel" style="background:<?php echo esc_attr($metas['color']); ?>"></span>
                    <span class="event_catgeory_name">
                      <a href="<?php echo get_category_link($category); ?>"
                         style="color:<?php echo esc_attr($metas['color']); ?>">
      ...
      <!-- Categories and tags -->
      <?php
      do_action('ecwd_tickets_events_single_meta', '', $ecwd_event_date_to, $post_id);
