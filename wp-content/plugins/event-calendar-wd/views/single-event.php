  ...
  if (!empty($event_categories)) {
    if (isset($event_categories[0])) {
      ...
      if (isset($metas['color'])) {
        $event_title_style = 'style = "color: ' . esc_attr($metas['color']) . '"';
      }
    }
  }
}

function print_embed_event() {
  require_once('embed_event.php');
}
      ...
      <article>
        <div class="ecwd-event" itemscope itemtype="http://schema.org/Event">
	      ...
	      <?php if ( ! post_password_required() ) { ?>
			  <div class="event-detalis">
				...
				<div class="ecwd-event-details">
				  ...
				  <?php
				  if ($ecwd_options['show_repeat_rate'] && !$is_default_dates) {
					$repeat_rate_text = $d->get_repeat_rate($post_id, '', $date_format);
					if ($repeat_rate_text != ''): ?>
					  <div class="ecwd_repeat_rate_text">
						<span><?php echo esc_html($repeat_rate_text); ?></span>
					  </div>
					<?php endif;
				  }
			  ...
			  if ($ecwd_event_show_map == 1 && $ecwd_event_latlong) {
				...
				$map_events[0]['zoom'] = intval($ecwd_event_zoom);
				...
				<div class="ecwd-event-video">
				  ...
				  if (strpos($ecwd_event_video, 'youtube') > 0) {
					...
					if (isset($video_array_of_vars['v']) && $video_array_of_vars['v']) {
					  ?>
					  <object data="https://www.youtube.com/v/<?php echo esc_html($video_array_of_vars['v']); ?>"
          ...
          if (!isset($ecwd_options['related_events']) || $ecwd_options['related_events'] == 1) {
            ...
            $start_date = ECWD::ecwd_date('Y-m-d', time());
...
if (isset($_GET['iframe']) && intval($_GET['iframe']) == 1) {
	...
	if ($ecwd_options['enable_sidebar_in_event'] == '1') {
		if (!isset($_GET['iframe']) || intval($_GET['iframe']) !== 1) {
			get_sidebar();
		}
	}
