...
function ecwd_get_settings_params(){
  ...
  $ecwd_settings = array(
    ...
    'general' => array(
      'clear_cache' => array(
        'id' => 'clear_cache',
        'name' => __('Clear cache', 'event-calendar-wd'),
        'desc' => __('Click to clear events cache pulled from other calendars.', 'event-calendar-wd'),
        'size' => 'small-text',
        'type' => 'link',
        'href' => $_SERVER['REQUEST_URI'] . '&ecwd_clear_cache=1'
      ),
      ...
      'time_zone' => array(
        ...
        'desc' => __('Use Continent/City format, e.g. Europe/Berlin. If left empty, the server timezone will be used (if set in php settings), otherwise default Europe/London. <a target="_blank" href="http://php.net/manual/en/timezones.php">PHP Timezones</a>', 'event-calendar-wd'),
        'size' => 'medium-text',
        'type' => 'text',
 ...
 /*     'date_format' => array(
  ...
  /*    'list_date_format' => array(
    ...
    'google_map' => array(
      ...
      'gmap_zoom' => array(
        'id' => 'gmap_zoom',
        'name' => __('Google Maps zoom', 'event-calendar-wd'),
        'desc' => '',
        'type' => 'number',
        'default' => 17
      ),
  ...
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_fb_settings', $ecwd_settings);
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_gcal_settings', $ecwd_settings);
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_ical_settings', $ecwd_settings);
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_add_event_settings', $ecwd_settings);
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_countdown_settings', $ecwd_settings);
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_af_settings', $ecwd_settings);
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_subscribe_settings', $ecwd_settings);
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_upcoming_events_settings', $ecwd_settings);
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_filter_settings', $ecwd_settings);
  $ecwd_settings = apply_filters(ECWD_PLUGIN_PREFIX . '_export_settings', $ecwd_settings);
}
...
function ecwd_get_settings_field_args($option, $section) {
    $settings_args = array(
        ...
        'show' => isset($option['show']) ? $option['show'] : array(),
        'hide' => isset($option['hide']) ? $option['hide'] : array(),
    );
...
function ecwd_status_select_callback($args) {
    ...
    $html = "\n" . '<select  id="ecwd_settings_' . $args['section'] . '[' . $args['id'] . ']" name="ecwd_settings_' . $args['section'] . '[' . $args['id'] . ']" >
        ...
        <option value="publish" ' . selected('publish', isset($ecwd_options[$args['id']]) ? $ecwd_options[$args['id']] : '', false) . '>Published</option>
        <option value="pending" ' . selected('pending', isset($ecwd_options[$args['id']]) ? $ecwd_options[$args['id']] : '', false) . '>Pending review</option>
    </select>' . "\n";
...
function ecwd_number_callback($args){
  global $ecwd_options;
  ...
  $html = "\n" . '<input type="number" min="0" class="' . $size . '" id="ecwd_settings_' . $args['section'] . '[' . $args['id'] . ']" name="ecwd_settings_' . $args['section'] . '[' . $args['id'] . ']" value="' . esc_attr($value) . '"/>' . "\n";
...
function ecwd_radio_callback($args) {
	...
	if(isset($args['show']) && count($args['show'])){ ?>
	<script>
      jQuery(document).ready(function ()
      {
        /*init*/
        var element_<?php echo $args["id"]; ?> = {
          name: "ecwd_settings_<?php echo $args['section'] . '[' . $args['id'] . ']'; ?>",
          show: [
            <?php
            foreach ($args['show'] as $key => $value) :
              echo "{key: '" . $key . "', val: [";
        ...
        ecwd_elements.radio_open(element_<?php echo $args["id"]; ?>);
        /*shor or hide on change*/
        jQuery('input[type=radio][name="ecwd_settings_<?php echo $args['section'] . '[' . $args['id'] . ']'; ?>"]').on("change", function ()
        {
          ecwd_elements.radio_open(element_<?php echo $args["id"]; ?>);
        });
      });
...
function ecwd_cats_checkbox_callback($args) {
    global $ecwd_options;
    $categories = get_terms('ecwd_event_category', array('hide_empty' => false));
    $html = '';
    if (!empty($categories)) {
 ...
 *
 */
function ecwd_af_text_callback($args) {
    global $ecwd_options;
    $value = isset($ecwd_options[$args['id']]) ? $ecwd_options[$args['id']] : array('');
    $size = ( isset($args['size']) && !is_null($args['size']) ) ? $args['size'] : '';
    $class = ( isset($args['class']) && !is_null($args['class']) ) ? $args['class'] : '';
    $html = '';
    foreach ($value as $item) {
        $html .= "\n" . '<input type="text" class="' . $size . " " . $class . '"  name="ecwd_settings_' . $args['section'] . '[' . $args['id'] . '][]" value="' . esc_attr($item) . '"/><div class="af_del">X</div>' . "<br>";
    }
...
function ecwd_get_settings() {
    ...
    if(empty($general_settings['events_font_style'])){
      $general_settings['events_font_style'] = 'italic';
    }
