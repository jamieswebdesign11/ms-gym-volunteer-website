...
class ECWD_Display {
    ...
    public function __construct($ids, $title_text = null, $sort_order = 'asc', $date = '', $page = 1, $search_params = array(), $displays = null, $filters = null, $page_items = 5, $event_search = 'yes', $display = '', $ecwd_views = array()) {
        ...
        $this->google_events_obj = new ECWD_Google_Events();
        $this->facebook_events_obj = new ECWD_Facebook_Events();
        $this->ical_events_obj = new ECWD_Ical_Events();
        ...
        if (!empty($ecwd_views) && isset($ecwd_views['calendar_head']) && $ecwd_views['calendar_head'] == "no") {
            $this->view_addon = true;
            $this->start_date = (isset($ecwd_views['start_date'])) ? $ecwd_views['start_date'] : "";
            $this->end_date = (isset($ecwd_views['end_date'])) ? $ecwd_views['end_date'] : "";
            $this->date = $this->end_date;
        } else {
            ...
            if (( isset($_REQUEST['t']) && $_REQUEST['t'] == 'week' ) || $this->display == 'week') {
                ...
                $this->date = ECWD::ecwd_date('Y-n-t', strtotime($this->end_date));
            } elseif (( isset($_REQUEST['t']) && $_REQUEST['t'] == 'list' ) || $this->display == 'list') {
                $offset = (isset($ecwd_options['period_for_list'])) ? $ecwd_options['period_for_list'] : 1;
                $this->end_date = ECWD::ecwd_date("Y-m-t", strtotime('+' . ($offset - 1) . ' months', strtotime($this->start_date)));
                $this->date = ECWD::ecwd_date('Y-n-t', strtotime($this->end_date));
            }
        }
        if ($this->id) {
    ...
    public function get_events() {
        ...
        foreach ($this->id as $id) {
            ...
            ECWD::add_private_posts($ecwd_events, $args);
            wp_reset_query();
            $google_events = array();
            $events = array();
            $ical_events = array();
            ...
            try {
                $google_events = $this->google_events_obj->get_google_events($id, $query);
            } catch (Exception $e) {
            ...
            try {
//                $facebook_events = $this->facebook_events_obj->get_facebook_events($id);
            } catch (Exception $e) {
            ...
            //fetch ical events
            try {
                $ical_events = $this->ical_events_obj->get_events($id);
            } catch (Exception $e) {
            ...
            $integrated_events = $google_events + $facebook_events + $ical_events;

            $integrated_events = $this->filter_integrated_events($integrated_events);

            $this->merged_events += $integrated_events + $events;
            ...
            if($hide_old_events === true) {
                $this->remove_past_events();
            }
        }
    }

  private function remove_past_events() {
    ...
    foreach ( $this->events as $k => $events ) {
      ...
      if ( !isset($filtered_events[$k]) ) {
        $filtered_events[$k] = array();
      }
      foreach ( $events as $k2 => $event ) {

        if ( strtotime($event['from']) >= strtotime(ECWD::ecwd_date("Y-m-d 00:00:00", time())) ) {
          $filtered_events[$k][$k2] = $event;
        }
      }
    }
    ...
    private function filter_integrated_events($integrated_events){
        if(empty($integrated_events)){
            return $integrated_events;
        }

        $filtered_integrated_events = array();

        if(isset($_REQUEST['ecwd_calendar_search']) && $_REQUEST['ecwd_calendar_search'] == '1' && !empty($_REQUEST['ecwd_query'])){
            $search_text = sanitize_text_field($_REQUEST['ecwd_query']);
            foreach ($integrated_events as $key=>$integrated_event) {
                if (stripos($integrated_event->title, $search_text) !== false || strpos($integrated_event->description, $search_text) !== false) {
                    $filtered_integrated_events[$key] = $integrated_event;
                }
            }
            return $filtered_integrated_events;
        }else{
            return $integrated_events;
        }
    }
    ...
    public function ecwd_content_excerpt_more($more){
        return sprintf(' ...');
    }
...
//        return $event2->start_time - $event1->start_time;
        ...
        foreach ($events as $id => $arr) {
            ...
            if ($metas && isset($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0]) && $metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] !== 'no_repeat' && $metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] != '') {
                ...
                if ($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] == 'weekly') {
                    ...
                    $how = ( isset($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_how'][0]) && $metas[ECWD_PLUGIN_PREFIX . '_event_repeat_how'][0] > 0 ? $metas[ECWD_PLUGIN_PREFIX . '_event_repeat_how'][0] : 1 );
                    if (count($days)) {
                   ...
                   /* if (strtotime($until) > strtotime($this->end_date)) {
                    ...
                    for ($i = 0; $i <= $eventdays; $i ++) {
                        ...
                        if (strtotime($until) >= strtotime(ECWD::ecwd_date('Y-m-d', strtotime($date)))) {
                            ...
                            $from = $date;
                            $to = ECWD::ecwd_date('Y-m-d', strtotime($from . ' + ' . $eventdayslong . ' days'));
                            if (!$current_month || (strtotime($from) <= strtotime($this->end_date) && strtotime($from) >= strtotime($this->start_date) && in_array(strtolower(ECWD::ecwd_date('l', strtotime($from))), $weekdays) )) {
    ...
    private function get_search_events_for_long_days(){
        ...
        foreach ($this->merged_events as $id=>$event) {
            if($event->metas['ecwd_event_repeat_event'][0] == "no_repeat"){
    ...
    public function get_events_for_long_days($events = '', $current_month = 1, $start_date = '', $end_date = '') {
        ...
        foreach ($events as $id => $arr) {
            ...
            if ($metas && isset($metas[ECWD_PLUGIN_PREFIX . '_event_venue'][0]) && $metas[ECWD_PLUGIN_PREFIX . '_event_venue'][0]) {
                ...
                $venue_id = intval($metas[ECWD_PLUGIN_PREFIX . '_event_venue'][0]);
                if (!isset($venues_cache[$venue_id])) {
                    $venue_post = get_post($venue_id);
                    if ($venue_post) {
                        $venue['name'] = $venue_post->post_title;
                        $venue['id'] = $venue_post->ID;
                        $venue['permalink'] = get_permalink($venue_post->ID);
                        $venues_cache[$venue_id] = $venue;
                    }
                }

                $venue = (isset($venues_cache[$venue_id])) ? $venues_cache[$venue_id] : array();
                ...
                if ($organizers_ids) {
                    foreach ($organizers_ids as $organizer_id) {
                        if ($organizer_id) {
                            ...
                            if(!isset($organizers_cache[$organizer_id])){
                                $opost = get_post($organizer_id);
                                if ($opost) {
                                    $organizers_cache[$organizer_id] = array(
                                      'id' => $opost->ID,
                                      'name' => $opost->post_title,
                                      'permalink' => get_permalink($opost->ID)
                                    );
                            ...
                            if(isset($organizers_cache[$organizer_id])){
                                $organizers[] = $organizers_cache[$organizer_id];
                                $organizersIDs[] = $organizer_id;
                            }
            ...
            $this->event_options['color'] = $color;
            $this->event_options['link'] = array_key_exists('link', $arr) ? $arr['link'] : '';
            $this->event_options['second_link'] = array_key_exists('url', $arr) ? $arr['url'] : '';//for integrated events
            $this->event_options['location'] = array_key_exists('location', $arr) ? $arr['location'] : '';
            $this->event_options['permalink'] = array_key_exists('permalink', $arr) ? $arr['permalink'] : '';
            $this->event_options['latlong'] = array_key_exists('latlong', $arr) ? $arr['latlong'] : '';
            $this->event_options['terms'] = $terms;
            ...
            if ($metas && isset($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0]) && $metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] !== 'no_repeat' && $metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] != '') {
                    ...
                    for ($i = 0; $i <= $eventdays; $i ++) {
                        ...
                        if (is_array($days) && in_array($week_day, $days)) {
                            ...
                            if (strtotime($until) >= strtotime(ECWD::ecwd_date('Y-m-d', strtotime($eventdate)))) {
                                ...
                                if (!$current_month || (strtotime($eventdate) <= strtotime($this->end_date) && strtotime($eventdate) >= strtotime($this->start_date) && in_array(strtolower(ECWD::ecwd_date('l', strtotime($eventdate))), $weekdays) )) {
                ...
                if(isset($arr["metas"]["exception_date"]) && isset($arr["metas"]["exception_date"][0])){
                  if(isset($this->events) && is_array($this->events)){
                    foreach($this->events as $key=>$val){
                      foreach($val as $key1=>$event){
                        if(isset($event["metas"]["exception_date"]) && isset($event["metas"]["exception_date"][0]) && !empty($event["metas"]["exception_date"][0])){
                          if($this->is_exception_date($event["from"], unserialize($event["metas"]["exception_date"][0]))){
                            unset($this->events[$key][$key1]);
                          }
                        }
                      }
                    }
    ...
    private function is_exception_date($date, $exception_date){
      $date = ECWD::ecwd_date("y-m-d", strtotime($date));
      if(is_array($exception_date) && !empty($date)){
        foreach($exception_date as $val){
          $edate = ECWD::ecwd_date("y-m-d", strtotime($val));
          if(strtotime($edate) === strtotime($date)){
            return true;
          }
        }
      }
    ...
    public function get_view ( $date = '', $type = '', $widget = 0, $ecwd_views = array(), $preview = '' ) {
        ...
        $cal = new Calendar($type, $date, '', $widget, $this->page_items, $this->page, $this->displays, $this->filters, $this->event_search, $ecwd_views, $preview);
        ...
        $html = $cal->showcal();
        if ($preview == false) {
            $html = apply_filters('ecwd_subscribe_form', $html, $this->id);
        }
        return $html;
    }
    ...
    public function get_repeat_rate($event_id, $metas = '', $datefomat = 'Y-m-d') {
        ...
        if ($metas && isset($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0]) && $metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] !== 'no_repeat' && $metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] != '') {
            ...
            if ($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] == 'daily') {
                if ($how > 1) {
                    $repeat_text .= __('Repeat every', 'event-calendar-wd') . ' ' . $how . ' ' . __('days', 'event-calendar-wd');
                } else {
                    $repeat_text .= __('Repeat every day', 'event-calendar-wd');
                }
            } elseif ($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] == 'weekly') {
                if ($how > 1) {
                    $repeat_text .= __('Repeat every', 'event-calendar-wd') . ' ' . $how . ' ' . __('weeks', 'event-calendar-wd');
                } else {
                    $repeat_text .= __('Repeat every week', 'event-calendar-wd');
                }


                if (count($days) > 0) {
                    $repeat_text .= ' ' . __('on', 'event-calendar-wd') . ' ';
                    foreach ($days as $i => $day) {
                        $repeat_text .= __(ucfirst($day), 'event-calendar-wd');
                        if ($i !== ( count($days) - 1 )) {
                            $repeat_text .= ',';
                        }
                        $repeat_text .= ' ';
                ...
                if ($how > 1) {
                    $repeat_text .= __('Repeat every', 'event-calendar-wd') . ' ' . $how . ' ' . __('months', 'event-calendar-wd');
                } else {
                    $repeat_text .= __('Repeat every month', 'event-calendar-wd');
                }
                $repeat_days = isset($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_month_on_days'][0]) ? $metas[ECWD_PLUGIN_PREFIX . '_event_repeat_month_on_days'][0] : 1;

                if ($repeat_days == 1) {
                    $repeat_text .=' ' . __('on the same day', 'event-calendar-wd');
                } else {
                    $repeat_when = isset($metas[ECWD_PLUGIN_PREFIX . '_monthly_list_monthly'][0]) ? $metas[ECWD_PLUGIN_PREFIX . '_monthly_list_monthly'][0] : false;
                    $repeat_day = isset($metas[ECWD_PLUGIN_PREFIX . '_monthly_week_monthly'][0]) ? $metas[ECWD_PLUGIN_PREFIX . '_monthly_week_monthly'][0] : false;
                    if ($repeat_when && $repeat_day) {
                        $repeat_text .= ' ' . __('on the', 'event-calendar-wd') . ucfirst($repeat_when) . ' ' . __(ucfirst($repeat_day), 'event-calendar-wd');
                    }
                }
            } elseif ($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_event'][0] == 'yearly') {
                if ($how > 1) {
                    $repeat_text .= __('Repeat every', 'event-calendar-wd') . ' ' . $how . ' ' . __('years', 'event-calendar-wd');
                } else {
                    $repeat_text .= __('Repeat every year', 'event-calendar-wd');
                }
                ...
                if ($repeat_days == 1) {
                    ...
                    $repeat_day = isset($metas[ECWD_PLUGIN_PREFIX . '_monthly_week_yearly'][0]) ? $metas[ECWD_PLUGIN_PREFIX . '_monthly_week_yearly'][0] : false;
                    if ($repeat_when && $repeat_day) {
                        $repeat_text .= ' ' . __('on the', 'event-calendar-wd') . ucfirst($repeat_when) . ' ' . ucfirst($repeat_day);
                    }
                }
            }
            $repeat_text .= ' ' . __('until', 'event-calendar-wd') . ' ' . ECWD::ecwd_date($datefomat, strtotime($metas[ECWD_PLUGIN_PREFIX . '_event_repeat_repeat_until'][0]));
        }
