...
class ECWD_Admin {
  ...
  protected static $default_shortcode = '[ecwd id="%s" type="full" page_items="5" event_search="yes" display="full" displays="full,list,week,day,map,4day,posterboard" filters=""]';
  private $importExports = array(
    "ecwd_import" => "ecwd.import/ecwd_import.php",
    "ecwd_gcal" => "ecwd-gcal/ecwd_gcal.php",
    "ecwd_ical" => "ecwd.ical/ecwd_calendar_ical.php",
    "ecwd_fb" => "ecwd-fb/ecwd_fb.php",
  );
  ...
  private function __construct() {
    ...
    add_action('admin_notices', array( $this, 'ecwd_addons_compatible_notice' ));
    ...
    add_action('init', array( $this, 'ecwd_recurring_events' ));
  }

  public function ecwd_recurring_events() {
    if ( !isset($_GET['ee']) ) {
      return;
    }
    include_once 'includes/events/ecwd-events-controller.php';
    $ev_controller = new ecwd_events_controller();
    $ev_controller->get_recurring_events("191", "2018-05-01", "2018-05-30");
    $ev_controller->get_recurring_events("206", "2018-05-01", "2018-05-30");
    $ev_controller->get_recurring_events("207", "2018-05-01", "2018-05-30");
    $ev_controller->get_recurring_events("208", "2018-05-01", "2018-05-30");
    die;
  }
  ...
  public function enqueue_block_editor_assets() {
    ...
    private function ecwd_page() {
        ...
        $ecwd_screen_ids = array(
          ...
          ECWD_PLUGIN_PREFIX . '_countdown_theme'
        );
        $ecwd_post_types = array(
          ECWD_PLUGIN_PREFIX . '_event',
        ...
        if (in_array($screen->id, $this->ecwd_page) || in_array($screen->post_type, $ecwd_post_types) || in_array($screen->id, $ecwd_screen_ids)) {
            return true;
        } else {
            return false;
        }
    ...
    static function activate() {
        ...
        if (
                ((!get_option('ecwd_setup_default_themes') || get_option('ecwd_setup_default_themes') != 1 ) && null === get_page_by_title('Default', 'OBJECT', 'ecwd_theme'))
                || count($themes) === 0
        ) {
            try {
                ECWD_Admin::add_pro_themes();
                update_option('ecwd_setup_default_themes', 1);
            } catch (Exception $e) {
                return;
            }
        }
        if (false === get_option('ecwd_grey_theme_id')) {
            ECWD_Admin::add_pro_themes('theme_6');
        }
        //Change free ecwd_calendar_theme meta with premium grey theme id.
        $free_calendars = get_posts(array(
            'post_type' => 'ecwd_calendar',
            'numberposts' => -1
        ));

        $grey_theme_id = get_option('ecwd_grey_theme_id');
        $defaul_theme = get_page_by_title('Grey', 'OBJECT', 'ecwd_theme');
        $defaul_theme_id = $defaul_theme->ID;
        if(!empty($free_calendars)) {
            foreach ($free_calendars as $calendar) {
                $theme_name = get_post_meta($calendar->ID, 'ecwd_calendar_theme', true);
                if (!ctype_digit($theme_name)) {
                    if ($theme_name == 'calendar') {
                        update_post_meta($calendar->ID, 'ecwd_calendar_theme', $defaul_theme_id);
                    } else {
                        update_post_meta($calendar->ID, 'ecwd_calendar_theme', $grey_theme_id);
                    }
                }
            }
        }
//recreate themes css files
        if (is_writable(ECWD_DIR . '/css/themes/')) {

            $type = ECWD_PLUGIN_PREFIX . '_theme';
            $args = array(
                'post_type' => $type,
                'post_status' => 'publish',
                'posts_per_page' => - 1,
            ...
            update_option('ecwd_themes_files_created', 0);
            $themes = get_posts($args);

            $them_op = array();
            if ($themes) {
                foreach ($themes as $theme) {
                    $theme_params = json_decode(get_post_meta($theme->ID, 'ecwd_theme_params', true), true);

                    if (is_array($theme_params)) {
                        ECWD_Cpt::generate_theme_file($theme->ID, $theme_params, $type);
                        $them_op[] = $theme->ID;
                    }
                }
            }
            update_option('ecwd_themes_files_created', $them_op);
        } else {
        ...
        if ($version_option == false || version_compare(substr($version_option, 2), '0.94', '<=')) {
            self::update_to_95($free_calendars);
        }
  ...
  public static function check_silent_update(){
    ...
    static function add_pro_themes($theme_id = null) {
        include_once( 'includes/ecwd-cpt-class.php' );
        $pro_themes = ECWD_Cpt::get_pro_themes();
        if ($theme_id != null && isset($pro_themes[$theme_id])) {
            $pro_themes = array($pro_themes[$theme_id]);
        }
        try {
            foreach ($pro_themes as $key => $theme) {
                ...
                if ($theme['name'] == 'Grey') {
                    update_option('ecwd_grey_theme_id', $new_post_id);
                }

                add_post_meta($new_post_id, 'ecwd_theme_params', $theme['params']);
                add_post_meta($new_post_id, 'ecwd_theme_name', $key);
            }
        } catch (Exception $e) {
            return;
        }
...
//    }
    ...
    public function add_plugin_admin_menu() {
        global $ecwd_config;
		$activeCount = 0;
		foreach($this->importExports as $key => $extention ){						
			if (is_plugin_active($extention)) {
				$activeCount++;
			}
		}
		...
		if($activeCount != 0){
			$this->ecwd_page[] = add_submenu_page(
			  ECWD_MENU_SLUG,
			  __('Import/Export', 'event-calendar-wd'),
			  __('Import/Export', 'event-calendar-wd'),
			  'manage_options',
			  $this->prefix . '_imports_exports',
      ...
      $this->ecwd_page[] = add_submenu_page(
        ...
        if ((defined('ECWD_TICKETING_NAME') && is_plugin_active(ECWD_TICKETING_NAME)) || (defined('ECWD_SUBSCRIBE_NAME') && is_plugin_active(ECWD_SUBSCRIBE_NAME))) {

            $this->ecwd_page[] = add_submenu_page(
              ECWD_MENU_SLUG,
              __('Mail Template', 'event-calendar-wd'),
              __('Mail Template', 'event-calendar-wd'),
              'manage_options',
              $this->prefix . '_mail_template',
              array($this, 'display_mail_template')
            );
    ...
    public function display_import_export_page() {
		$activeAddon = array();
		$addonsName = array(
			"ecwd_import" => "Import/Export",
			"ecwd_gcal" => "Import Gcal",
			"ecwd_ical" => "Import Ical",
			"ecwd_fb" => "Import FB events"
		);
		foreach($this->importExports as $key => $extention ){						
			if (is_plugin_active($extention)) {
				$activeAddon[] = $key;
		...
		$import_addon = (isset($_GET["import_addon"]) && $_GET["import_addon"] != "") ? $_GET["import_addon"] : $activeAddon[0];
		echo '<div id="ecwd-export-import">
			 <h2>' . esc_html( get_admin_page_title() ) . '</h2>';
		if(count($activeAddon) >= 2){
			$tabs = '<h2 class="nav-tab-wrapper">';
			    foreach($activeAddon as $addon){
					$class = ($import_addon == $addon) ? "nav-tab-active" : "";
					$tabs .= '<a class="nav-tab ' . $class . '" href="edit.php?post_type=ecwd_event&amp;page=ecwd_imports_exports&amp;import_addon=' . $addon . '">' . (isset($addonsName[$addon]) ? $addonsName[$addon] : "") . '</a>';
				} 	
			$tabs .= '</h2>';
			echo $tabs;
		}
		echo '<h2>' . (isset($addonsName[$import_addon]) ? $addonsName[$import_addon] : "") . '</h2>';
		$filter = $import_addon . "_tab";
    ...
    public function display_mail_template() {
        $this->mail_template = apply_filters($this->prefix . '_mail_template', $this->mail_template);

        if (isset($_GET['type'])) {
            $type = sanitize_text_field($_GET['type']);

            if (isset($this->mail_template[$type])) {
                $option = get_option($this->mail_template[$type]['option_name']);
                if ((isset($_POST['ecwd_template']) && $_POST['reset_template'] == 'Reset') || $option == false) {
                    do_action('ecwd_reset_template', $type);
                }

                $this->ecwd_edit_template($type);
            }
        } else {
            include_once('views/admin/ecwd-mail-template-list.php');
        }
    }
  ...
  /**
    ...
    public function enqueue_admin_styles() {
        ...
        if ($this->ecwd_page()) {
            ...
            wp_enqueue_style($this->prefix . '-admin-css', plugins_url('css/admin/admin.css', __FILE__), array(), $styles_key, 'all');
            ...
            wp_enqueue_style($this->prefix . '-admin-datetimepicker-css', plugins_url('css/admin/jquery.datetimepicker.css', __FILE__), array(), $styles_key, 'all');
            wp_enqueue_style($this->prefix . '-admin-colorpicker-css', plugins_url('css/admin/evol.colorpicker.css', __FILE__), array(), $styles_key, 'all');
    ...
    public function enqueue_admin_scripts() {
          ...
          'ajaxnonce' => wp_create_nonce('ecwd_ajaxnonce')
				));
				...
				if ( (isset($ecwd_options['gmap_type']) && $ecwd_options['gmap_type'] == '1') ) {
						$params['gmap_type'] = 'SATELLITE';
				} else {
						$params['gmap_type'] = 'ROADMAP';
				}
      ...
      if (!$this->ecwd_page() || $screen->post_type == "ecwd_calendar") {
        wp_localize_script('jquery', 'ecwd_translate', array(
          ...
          'ecwd_view_5'=>__("View 5", 'event-calendar-wd'),
          'ecwd_view_6'=>__("View 6", 'event-calendar-wd'),
          'ecwd_view_7'=>__("View 7", 'event-calendar-wd'),
          'ecwd_view_activate_filters'=>__("Get and activate filters add-on", 'event-calendar-wd'),

          'ecwd_filter_1'=>__("Filter 1", 'event-calendar-wd'),
          'ecwd_filter_2'=>__("Filter 2", 'event-calendar-wd'),
          'ecwd_filter_3'=>__("Filter 3", 'event-calendar-wd'),
          'ecwd_filter_4'=>__("Filter 4", 'event-calendar-wd'),
          'ecwd_filter_5'=>__("Filter 5", 'event-calendar-wd'),
    ...
    public function admin_head() {
        ...
        ECWD::add_private_posts($calendar_posts, $args);
        $args = array(
        ...
        ECWD::add_private_posts($event_posts, $args);
        ...
        <?php } ?>
                    ...
                    {text: '<?php _e("4 Days", "event-calendar-wd");?>', value: '4day'},
                    {text: '<?php _e("Posterboard", "event-calendar-wd");?>', value: 'posterboard'},
                    {text: '<?php _e("Map", "event-calendar-wd");?>', value: 'map'}
                    ],
                    'ecwd_filters': [
                    {text: '<?php _e("None", "event-calendar-wd");?>', value: 'none'},
                    {text: '<?php _e("Days", "event-calendar-wd");?>', value: 'days'},
                    {text: '<?php _e("Categories", "event-calendar-wd");?>', value: 'categories'},
                    {text: '<?php _e("Tags", "event-calendar-wd");?>', value: 'tags'},
                    {text: '<?php _e("Venues", "event-calendar-wd");?>', value: 'venues'},
                    {text: '<?php _e("Organizers", "event-calendar-wd");?>', value: 'organizers'}
                    ]
            };
        </script>
        <!-- TinyMCE Shortcode Plugin -->
...
// add the button to the tinyMCE bar
  ...
  public function ecwd_addons_compatible_notice(){
    $plugins = get_plugins();


    $addons = array(
      'ecwd-frontend-event-management/ecwd_add_event.php' => array(
        'version' => '1.0.13'
      ),
      'ecwd-additional-fields/ecwd_additional_fields.php' => array(
        'version' => '1.0.3'
      ),
      'ecwd-subscribe/ecwd_subscribe.php' => array(
        'version' => '1.0.6'
      ),
      'ecwd-export/ecwd_export.php' => array(
        'version' => '1.0.7'
      ),
	  'ecwd-export/ecwd_import.php' => array(
        'version' => '1.0.10'
      ),
      'ecwd-filters/ecwd_filters.php' => array(
        'version' => '1.0.4'
      ),
      'ecwd-fb/ecwd_fb.php' => array(
        'version' => '1.0.6'
      ),
      'ecwd.ical/ecwd_calendar_ical.php' => array(
        'version' => '1.0.7'
      ),
      'ecwd-gcal/ecwd_gcal.php' => array(
        'version' => '1.0.6'
      ),
    );

    $not_compatible_addons = array();
    foreach($addons as $file_name => $info) {
      if(isset($plugins[$file_name]) && is_plugin_active($file_name)) {

        if(version_compare($plugins[$file_name]['Version'], $info['version'], '<')) {
          $incompat_plugin = $plugins[$file_name];
          $incompat_plugin['Version'] = $info['version'];

          $not_compatible_addons[] = $incompat_plugin;
        }
    ...
    if(!empty($not_compatible_addons)){

      $notices = '';
      foreach($not_compatible_addons as $addon) {
        $notice = sprintf('Event Calendar WD plugin requires version %s or higher of %s extension. Please update %s extension.', $addon['Version'], $addon['Title'], $addon['Title']);
        $notices .= '<p>' . $notice . '</p>';
      }
    ...
    /**
     * Call if any ecwd plugin updated.
     */
    public static function plugin_updated() {
        delete_transient('ecwd_update_check');
    }
